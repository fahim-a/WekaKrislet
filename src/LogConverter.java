...
public class LogConverter {
	...
	private String[] actions = Property.getInstance().getProperty("actions")
			.split(";");
	private String[] attributes = Property.getInstance()
			.getProperty("attributes").split(";");
	private String[] attributes_types = Property.getInstance()
			.getProperty("attributes_types").split(";");
	 ...
	 * @throws IOException
	 */
	public void createWekaData(String logFile, String wekaOutputFile)
			throws IOException {
			...
			if (message.startsWith("(see")) {
				...
				for (String ac : this.actions) {
					if (message.startsWith("(" + ac)) {
						...
						System.out.println(di.getMsg_number() + " - "
								+ di.getARFFmessage());
						di = null;
						break;
					}
	...
	private String getARFFHeader() throws Exception {
		...
		sb.append("@RELEATION "
				+ Property.getInstance().getProperty("weka_relation_name"));
		sb.append("\n");
		sb.append("\n");
