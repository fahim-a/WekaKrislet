...
public class DataInstances {
	...
	private String msg_number = null; // in a visual message such as '(see 88
										// ((f l t) 66 -5)...etc ', the message
										// number is 88
	private String[] actions = Property.getInstance().getProperty("actions")
			.split(";");

	public DataInstances(VisualInfo vi) {
		this.vi = vi;
		this.my_side = Property.getInstance().getProperty("player_side")
				.charAt(0);
		...
		if (vi != null) {
			...
			if (this.my_side == 'l') {
				if (getObject("goal r") != null) {
					net_distance = Float
							.toString(getObject("goal r").m_distance);
					net_direction = Float
							.toString(getObject("goal r").m_direction);
				} else {
					net_distance = "?";
					net_direction = "?";
				}
				...
				if (getObject("goal l") != null) {
					net_distance = Float
							.toString(getObject("goal l").m_distance);
					net_direction = Float
							.toString(getObject("goal l").m_direction);
				} else {
					net_distance = "?";
					net_direction = "?";
				}
			...
			for (ObjectInfo i : players) {
				...
				sb.append(ball_distance
						...
						+ net_direction
						+ ","
						+ Float.toString(p.m_distance)
						+ ","
						+ Float.toString(p.m_direction)
						+ ","
						+ (p.m_teamName.isEmpty() ? "?" : (p.m_teamName
